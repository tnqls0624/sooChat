...
let RoomRepository = class RoomRepository {
    ...
    async joinRoom(roomId, existingRoomMembersLength) {
        const queryRunner = this.connection.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
        try {
            ...
            await queryRunner.manager.update(room_entity_1.Room, [{ id, total: existingRoomMembersLength + 1 }], ["id"]);
        }
        catch (error) {
            await queryRunner.rollbackTransaction();
            throw new common_1.HttpException(error, 400);
        ...
        console.log("방참가!:", roomId, existingRoomMembersLength);
    }
...
/******/ (() => {
/******/ 	__webpack_require__.h = () => ("3109a6d2e898a239bdd7")
/******/ })();
/******/ 
/******/ }
;